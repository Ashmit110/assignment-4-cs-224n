...
class CharCorruptionDataset(Dataset):
    def __init__(self, data, block_size):
        ...
        self.PAD_CHAR = "\u25A1" # the empty square character, for pawhatd
    ...
    def __getitem__(self, idx):
        ...
        masked_content_length = random.randint(1, trunc_length // 2)
        prefix_length = random.randint(0, max(0, trunc_length - masked_content_length - 1))
        suffix_length = trunc_length - prefix_length - masked_content_length

        prefix = truncated_doc[:prefix_length]
        masked_content = truncated_doc[prefix_length:prefix_length + masked_content_length]
        suffix = truncated_doc[prefix_length + masked_content_length:]
        ...
        x = torch.tensor([self.stoi.get(ch, self.stoi[self.PAD_CHAR]) for ch in x_str], dtype=torch.long)
        y = torch.tensor([self.stoi.get(ch, self.stoi[self.PAD_CHAR]) for ch in y_str], dtype=torch.long)
