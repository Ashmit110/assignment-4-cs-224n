...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    temp=torch.zeros(max_positions,dim//2,2)
    for t in range(max_positions):
        for i in range(dim//2):
            theta_i = 10000**((-2*(i-1))/dim)
            
            temp[t,i,0]=math.cos(t*theta_i)
            temp[t,i,1]=math.sin(t*theta_i)
                
    rope_cache=temp
    ### END YOUR CODE ###
    return rope_cache    #shape (max_positions, dim/2, 2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    rope_complex=torch.view_as_complex(rope_cache) #shape (T, dim/2)
    even_hs_indices = torch.arange(0, x.size(-1), 2)
    x_real=x[..., even_hs_indices]#shape (B,nh,T, dim/2)
    odd_hs_indices = torch.arange(1, x.size(-1), 2)
    x_imaginary=x[..., odd_hs_indices]#shape (B,nh,T, dim/2)
    
    
    x_cache=torch.complex(x_real,x_imaginary)#shape (B,nh,T, dim/2)
    encoded_complex=x_cache*rope_complex
    encoded_rational=torch.view_as_real(encoded_complex)#shape (B,nh,T, dim/2,2)
    
    x_even = encoded_rational[..., 0]  # Real part at even positions, shape (B, nh, T, dim/2)
    x_odd = encoded_rational[..., 1]   # Imaginary part at odd positions, shape (B, nh, T, dim/2)

    rotated_x=torch.empty((B, nh, T, hs), dtype=encoded_rational.dtype)
    
    # Assign even indices
    rotated_x[..., ::2] = x_even  # Place x_even at even positions
    # Assign odd indices
    rotated_x[..., 1::2] = x_odd  # Place x_odd at odd positions
    pass
    ### END YOUR CODE ###
    return rotated_x.to("cuda")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att = att.masked_fill((self.mask[:,:,:T,:T] == 0).to('cuda'), -1e10)
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        
        # print(f"x is on device{x.device}")
        # print(f"k is on device{k.device}")
        # print(f"q is on device{q.device}")
        # print(f"v is on device{v.device}")
        # print(f"att is on device{att.device}")
